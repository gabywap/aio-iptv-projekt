...
// =========================
// Support drawer + mobile nav drawer (FAB)
// =========================
...
function initMobileNavDrawer() {
  ...
  closeTriggers.forEach(el => {
    el.addEventListener('click', (e) => {
      // Allow anchor navigation to proceed, but close drawer immediately
      closeDrawer();
    });
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try { initSupportDrawer(); } catch (e) { console.warn(e); }
  try { initMobileNavDrawer(); } catch (e) { console.warn(e); }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // If we have the new mobile drawer, its handlers are attached in initMobileNavDrawer()
    // Keep this legacy toggler only for older templates.
    if (drawer) {
        // no-op here
    } else if (nav && toggle) {
